...
//total=4 bytes
void DW1000Mac::generateShortBlinkFrame(byte frame[], byte sourceShortAddress[])
{
    //Frame Control
    *frame=FC_1_BLINK;
    //sequence number
    *(frame+1)=_seqNumber;
    ...
    byte sourceShortAddressReverse[2];
    reverseArray(sourceShortAddressReverse, sourceShortAddress,2);
    memcpy(frame+2, sourceShortAddressReverse, 2);
...
void DW1000Mac::decodeShortBlinkFrame(byte frame[], byte shortAddress[])
{
    byte reverseShortAddress[2];
    memcpy(reverseShortAddress, frame+2, 2);
