...
// pins
int DW1000Class::_ss = -1;
int DW1000Class::_rst = -1;
int DW1000Class::_irq = -1;
// IRQ callbacks
...
unsigned int DW1000Class::_antennaDelay = DW1000_DEFAULT_ANTENNA_DELAY;
...
// modes of operation
const byte DW1000Class::MODE_IN_CIRCUIT_1[] = {TRX_RATE_850KBPS, TX_PULSE_FREQ_64MHZ, TX_PREAMBLE_LEN_1024, CHANNEL_4};
const byte DW1000Class::MODE_IN_CIRCUIT_2[] = {TRX_RATE_850KBPS, TX_PULSE_FREQ_16MHZ, TX_PREAMBLE_LEN_1024, CHANNEL_4};
const byte DW1000Class::MODE_IN_CIRCUIT_3[] = {TRX_RATE_6800KBPS, TX_PULSE_FREQ_64MHZ, TX_PREAMBLE_LEN_1024, CHANNEL_4};
const byte DW1000Class::MODE_IN_CIRCUIT_4[] = {TRX_RATE_6800KBPS, TX_PULSE_FREQ_16MHZ, TX_PREAMBLE_LEN_1024, CHANNEL_4};
const byte DW1000Class::MODE_IN_CIRCUIT_5[] = {TRX_RATE_850KBPS, TX_PULSE_FREQ_64MHZ, TX_PREAMBLE_LEN_512, CHANNEL_4};
const byte DW1000Class::MODE_IN_CIRCUIT_7[] = {TRX_RATE_850KBPS, TX_PULSE_FREQ_64MHZ, TX_PREAMBLE_LEN_256, CHANNEL_4};
...
// SPI settings
// Use fastest SPI available up to DW1000 max speed of 20MHz
...
/*void DW1000Class::sleepToReceive()
{
  uint8_t reg[LEN_AON_CTRL>LEN_AON_CFG0?LEN_AON_CTRL:LEN_AON_CFG0];
  
  readBytes(AON, AON_WCFG_SUB, reg, LEN_AON_WCFG);
  setBit(reg, LEN_AON_WCFG, ONW_RX_BIT, 1);
  setBit(reg, LEN_AON_WCFG, ONW_LDC_BIT, 1);
  setBit(reg, LEN_AON_WCFG, ONW_LLDE_BIT, 1);
  writeBytes(AON, AON_WCFG_SUB, reg, LEN_AON_WCFG);
  
  readBytes(AON, AON_CFG0_SUB, reg, LEN_AON_CFG0);
  setBit(reg, LEN_AON_CFG0, SLEEP_EN_BIT, 1);
  setBit(reg, LEN_AON_CFG0, WAKE_CNT_BIT, 1);
  writeBytes(AON, AON_CFG0_SUB, reg, LEN_AON_CFG0);
  
  readBytes(AON, AON_CTRL_SUB, reg, LEN_AON_CTRL);
  setBit(reg, LEN_AON_CTRL, UPL_CFG_BIT, 1);
  writeBytes(AON, AON_CTRL_SUB, reg, LEN_AON_CTRL);
}*/

void DW1000Class::goToDeepSleep(bool preserveRegs, bool wakeToRx)
{
  uint8_t reg[4];
  
  idle();
  clearAllStatus(); // clear all event status

   // DW1000 user manual page 16:
   //DW1000 should issue interrupt when going from wakeup to init state
   //todo: seems not to work
//   setBit(_sysmask, LEN_SYS_MASK, SLP2INIT_BIT, 1);
//   writeSystemEventMaskRegister();

  // reset UPL_CFG-Bit
  readBytes(AON, AON_CTRL_SUB, reg, LEN_AON_CTRL);
  setBit(reg, LEN_AON_CTRL, UPL_CFG_BIT, 0);
  writeBytes(AON, AON_CTRL_SUB, reg, LEN_AON_CTRL);
  ...
  {
    if (wakeToRx)
      setBit(reg, LEN_AON_WCFG, ONW_RX_BIT, 1); // Enter RX after WakeUp / dw1000 is usually in INIT after wakeup
    setBit(reg, LEN_AON_WCFG, ONW_LDC_BIT, 1);  // reload preserved register contents
    setBit(reg, LEN_AON_WCFG, ONW_LLDE_BIT, 1); // Load timestamp microcode after wakeup
    //setBit(reg, LEN_AON_WCFG, ONW_LEUI_BIT, 1); //  load the EUI from OTP memory / currently not needed
  }
  writeBytes(AON, AON_WCFG_SUB, reg, LEN_AON_WCFG);

  // DW1000 manual, page 156:
  // This bit needs to be set to 0 for correct operation in the SLEEP state
  // within the DW1000. By default this bit is set to 1.  The host system should
  // set this bit to zero as part of initialisation or controlling the entry
  // of the DW1000 into the SLEEP state to ensure correct operation of the
  // SLEEP mode
  readBytes(AON, AON_CFG1_SUB, reg, LEN_AON_CFG1);
  setBit(reg, LEN_AON_CFG1, SLEEP_CEN_BIT, 0); // disable sleep counter
  setBit(reg, LEN_AON_CFG1, SMXX_BIT, 0);
  writeBytes(AON, AON_CFG1_SUB, reg, LEN_AON_CFG1);

  // enable sleep
  readBytes(AON, AON_CFG0_SUB, reg, LEN_AON_CFG0);
  setBit(reg, LEN_AON_CFG0, WAKE_SPI_BIT, 0);
  setBit(reg, LEN_AON_CFG0, WAKE_CNT_BIT, 0);
  setBit(reg, LEN_AON_CFG0, WAKE_PIN_BIT, 1);
  setBit(reg, LEN_AON_CFG0, SLEEP_EN_BIT, 1);
  writeBytes(AON, AON_CFG0_SUB, reg, LEN_AON_CFG0);
  
  // save config to AON
  readBytes(AON, AON_CTRL_SUB, reg, LEN_AON_CTRL);
  setBit(reg, LEN_AON_CTRL, SAVE_BIT, 1);
  writeBytes(AON, AON_CTRL_SUB, reg, LEN_AON_CTRL);
  
  // Datasheet 2.4.1.2: Do not access SPI after triggering AON save.
  // Should be completed after around 7us. Eliminate any risk, wait some extra time.
...
void DW1000Class::reselect(int ss) {
	_ss = ss;
	if (_ss>=0) pinMode(_ss, OUTPUT);
	if (_ss>=0) digitalWrite(_ss, HIGH);
}
...
void DW1000Class::begin(int ss, int irq, int rst) {
	...
	if (_ss>=0) digitalWrite(_ss, HIGH);
	// attach interrupt
	if (_irq>=0) attachInterrupt(_irq, DW1000Class::handleInterrupt, RISING);
}
...
void DW1000Class::tune() {
  ...
  // force average value so later calibration is possible
	writeValueToBytes(fsxtalt, 0x6F, LEN_FS_XTALT);
...
void DW1000Class::handleInterrupt()
{
  ...
  clearAllStatus();
    ...
    // fixme todo
    // starting receive two times is not the best way.
    // currently the only way to prevent chip freeze.
...
void DW1000Class::newReceive(bool clearStatus) {
	idle();
	memset(_sysctrl, 0, LEN_SYS_CTRL);
	if (clearStatus) clearReceiveStatus();
	_deviceMode = RX_MODE;
}
...
void DW1000Class::startTransmit() {
	...
	writeBytes(SYS_CTRL, NO_SUB, _sysctrl, LEN_SYS_CTRL);
// receive will be reenabled in TX complete isr. Doing it the old way led to chip freezes.
// 	if(_permanentReceive) {
// 		memset(_sysctrl, 0, LEN_SYS_CTRL);
// 		_deviceMode = RX_MODE;
// 		startReceive();
...
void DW1000Class::writeAntennaDelay()
{
  byte antennaDelayBytes[LEN_TX_ANTD>LEN_LDE_RXANTD?LEN_TX_ANTD:LEN_LDE_RXANTD];
  writeValueToBytes(antennaDelayBytes, _antennaDelay, LEN_TX_ANTD>LEN_LDE_RXANTD?LEN_TX_ANTD:LEN_LDE_RXANTD);
  writeBytes(TX_ANTD, NO_SUB, antennaDelayBytes, LEN_TX_ANTD);
  writeBytes(LDE_IF, LDE_RXANTD_SUB, antennaDelayBytes, LEN_LDE_RXANTD);
}

void DW1000Class::setAntennaDelay(unsigned short newDelay)
{
  _antennaDelay = newDelay;
  writeAntennaDelay();
}
...
void DW1000Class::commitConfiguration() {
  ...
  writeSystemEventMaskRegister();
	...
	//writeValueToBytes(antennaDelayBytes, 0x00, LEN_STAMP);
...
DW1000Time DW1000Class::setDelay(const DW1000Time& delay) {
	...
	futureTime.addTimestamp(_antennaDelay);
	return futureTime;
}
...
void DW1000Class::setPreambleLength(byte prealen) {
	prealen &= 0x0F;
	_txfctrl[2] &= 0xC3;
	_txfctrl[2] |= (byte)((prealen << 2) & 0x3C);
...
boolean DW1000Class::isWakeupEvent() {
    return getBit(_sysstatus, LEN_SYS_STATUS, SLP2INIT_BIT);
}


void DW1000Class::clearAllStatus() {
  byte tBuf[LEN_SYS_STATUS];
	memset(tBuf, 0xFF, LEN_SYS_STATUS);
	writeBytes(SYS_STATUS, NO_SUB, tBuf, LEN_SYS_STATUS);
}

void DW1000Class::clearReceiveTimestampAvailableStatus() {
  byte tBuf[LEN_SYS_STATUS];
  memset(tBuf, 0, LEN_SYS_STATUS);
	setBit(tBuf, LEN_SYS_STATUS, LDEDONE_BIT, true);
	writeBytes(SYS_STATUS, NO_SUB, tBuf, LEN_SYS_STATUS);
}
...
void DW1000Class::clearReceiveStatus() {
  ...
  memset(tBuf, 0, LEN_SYS_STATUS);
	...
	setBit(tBuf, LEN_SYS_STATUS, RXDFR_BIT, true);
	setBit(tBuf, LEN_SYS_STATUS, LDEDONE_BIT, true);
	setBit(tBuf, LEN_SYS_STATUS, LDEERR_BIT, true);
	setBit(tBuf, LEN_SYS_STATUS, RXPHE_BIT, true);
	setBit(tBuf, LEN_SYS_STATUS, RXFCE_BIT, true);
	setBit(tBuf, LEN_SYS_STATUS, RXFCG_BIT, true);
	setBit(tBuf, LEN_SYS_STATUS, RXRFSL_BIT, true);
	writeBytes(SYS_STATUS, NO_SUB, tBuf, LEN_SYS_STATUS);
}
...
void DW1000Class::clearTransmitStatus() {
  ...
  memset(tBuf, 0, LEN_SYS_STATUS);
	...
	setBit(tBuf, LEN_SYS_STATUS, TXFRB_BIT, true);
	setBit(tBuf, LEN_SYS_STATUS, TXPRS_BIT, true);
	setBit(tBuf, LEN_SYS_STATUS, TXPHS_BIT, true);
	setBit(tBuf, LEN_SYS_STATUS, TXFRS_BIT, true);
	writeBytes(SYS_STATUS, NO_SUB, tBuf, LEN_SYS_STATUS);
}
...
//	Kills Processor when a millis is running. Seems not necessary at all.
//	delayMicroseconds(5);
	if (_ss>=0) digitalWrite(_ss,HIGH);
